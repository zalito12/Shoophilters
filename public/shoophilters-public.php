...
namespace GonGarceIO\Shoophilters\Public;
...
class Shoophilters_Public {
	 ...
	 *
	 * @since    1.0.0
	 * @param      string $shoophilters       The name of the plugin.
	...
	public function load() {
		add_action( 'woocommerce_before_template_part', array( $this, 'shoophilters_wc_before_template_part' ), 1 );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->shoophilters, plugin_dir_url( __FILE__ ) . 'js/shoophilters-public.js', array( 'jquery', 'jquery-ui-slider', 'wp-i18n', 'wp-a11y' ), $this->version, false );

		wp_add_inline_script(
			$this->shoophilters,
			'const SHOOPHILTERS = ' . wp_json_encode(
				array(
	 ...
	 */
	public function shoophilters_wc_before_template_part( $template_name ) {
		if ( 'loop/result-count.php' === $template_name ) {
			add_filter( 'ngettext', array( $this, 'shoophilters_load_more_products_count_additional' ), 1, 9999 );
			add_filter( 'ngettext_with_context', array( $this, 'shoophilters_load_more_products_count_additional' ), 1, 9999 );
		}
	}
	 ...
	 */
	public function shoophilters_load_more_products_count_additional( $gettext ) {
		remove_filter( 'ngettext', array( $this, 'shoophilters_load_more_products_count_additional' ), 1, 9999 );
		remove_filter( 'ngettext_with_context', array( $this, 'shoophilters_load_more_products_count_additional' ), 1, 9999 );
		...
		echo '<span class="shoophilters-result-count" style="display: none;" data-total="' . esc_attr( $total ) . '" data-first="' . esc_attr( $first ) . '" data-last="' . esc_attr( $last ) . '">';
		if ( 1 === $total ) {
